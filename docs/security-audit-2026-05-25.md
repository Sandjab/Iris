...
|---|:---:|:---:|:---:|:---:|
| Total findings | 0 | 4 | 11 | 4 |

> *Correction 2026-05-25 post-review* : un finding 🔵 Low initial (D.3 — JSONDecoder depth limit) a été retiré après vérification du code source `swift-foundation` qui montre une protection native à 512 niveaux. Voir Domaine D pour la note détaillée.
...
### ~~🔵 Low — JSONDecoder sans depth limit~~ — **Non-finding (correction 2026-05-25)**

**Référence** : `Sources/IrisKit/IPC/JSONRPC.swift:248-252`

**Correction post-review Gemini** : ce finding était factuellement incorrect dans la version initiale de l'audit. Vérification a posteriori :

- Le parser JSON de Foundation (`swift-foundation/Sources/FoundationEssentials/JSON/JSONScanner.swift`) **hardcode un dépth limit de 512**. Code source vérifié sur le repo officiel `swiftlang/swift-foundation` :
  ```swift
  // scanArray() et scanObject()
  guard self.depth < 512 else {
      throw JSONError.tooManyNestedArraysOrDictionaries(...)
  }
  ```
- Au-delà de 512 niveaux d'imbrication, le scanner lève `JSONError.tooManyNestedArraysOrDictionaries`, que `JSONDecoder` surface aux consumers sous forme de `DecodingError`.
- Cette protection native suffit pour le threat model de IRIS (DoS par stack exhaustion bloqué).

Petite nuance par rapport à la remarque Gemini : il n'existe pas de propriété publique nommée `decodingDepthLimit` exposée à l'API. La limite est hardcoded à 512 dans le scanner et n'est pas paramétrable côté consumer. Mais le résultat fonctionnel est identique : la protection est intégrée.

**Conclusion** : pas d'action requise. La protection contre les JSON profondément imbriqués est déjà en place via le scanner Foundation. Pas de fix Phase 10.
